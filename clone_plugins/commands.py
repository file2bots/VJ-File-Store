...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('💝 Sᴜʙsᴄʀɪʙᴇ Mʏ Cʜᴀɴɴᴇʟ', url='https://t.me/PK_LinkZz')
            ],[
            InlineKeyboardButton('🤖 Cʀᴇᴀᴛᴇ Yᴏᴜʀ Oᴡɴ Cʟᴏɴᴇ Bᴏᴛ', url=f'https://t.me/{BOT_USERNAME}?start=clone')
            ],[
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@PK_LinkZz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
